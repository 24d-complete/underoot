...
function activate(extensionContext) {
    ...
    extensionContext.subscriptions.push(vscode.workspace.onDidSaveTextDocument((e) => {
        ...
        if (lw_1.lw.file.hasLaTeXLangId(e.languageId) || lw_1.lw.file.hasLaTeXClassPackageLangId(e.languageId) ||
            lw_1.lw.cache.getIncludedTeX(lw_1.lw.root.file.path).has(e.fileName) ||
            ...
            if (lw_1.lw.compile.isFileExcludedFromBuildOnSave(e.fileName)) {
        ...
        if (lw_1.lw.file.hasLaTeXLangId(e.languageId)) {
    ...
    extensionContext.subscriptions.push(vscode.window.onDidChangeActiveTextEditor(async (e) => {
        ...
        if (vscode.window.visibleTextEditors.filter(editor => lw_1.lw.file.hasLaTeXWorkshopLangId(editor.document.languageId)).length > 0) {
            logger.showStatus();
        ...
        if (e && (lw_1.lw.file.hasLaTeXLangId(e.document.languageId)
            || lw_1.lw.file.hasBibLangId(e.document.languageId)
            || lw_1.lw.file.hasLaTeXClassPackageLangId(e.document.languageId))) {
            if (!lw_1.lw.file.hasBibLangId(e.document.languageId) && (e.document.fileName !== lw_1.lw.previousActive?.document.fileName)) {
        ...
        if (lw_1.lw.file.hasLaTeXLangId(e.document.languageId)
            || lw_1.lw.file.hasBibLangId(e.document.languageId)
    ...
    extensionContext.subscriptions.push(vscode.workspace.onDidChangeTextDocument((e) => {
        ...
        if (!lw_1.lw.file.hasLaTeXWorkshopLangId(e.document.languageId)) {
            return;
        }
    ...
    extensionContext.subscriptions.push(vscode.window.onDidChangeTextEditorSelection((e) => {
        if (lw_1.lw.file.hasLaTeXLangId(e.textEditor.document.languageId) ||
    ...
    void lw_1.lw.root.find().then(() => {
        lw_1.lw.lint.latex.root();
        if (lw_1.lw.file.hasLaTeXLangId(vscode.window.activeTextEditor?.document.languageId ?? '')) {
            lw_1.lw.previousActive = vscode.window.activeTextEditor;
        }
    });
    conflictCheck();
    // --- Underoot: TeX Environment Detection ---
    const texEnv = require('./compile/tex-environment');
    const tinyTexInstaller = require('./compile/tinytex-installer');
    texEnv.detectTexEnvironment().then(env => {
        if (env.texDistro === 'none') {
            tinyTexInstaller.promptInstallTinyTeX(extensionContext);
        }
    });

    // --- Underoot: Engine Manager ---
    const engineManager = require('./compile/engine-manager');
    engineManager.initialize(extensionContext);
    // --- End Underoot ---
...
function registerProviders(extensionContext) {
    ...
    extensionContext.subscriptions.push(vscode.languages.registerHoverProvider(latexSymbolSelector, lw_1.lw.preview.provider), vscode.languages.registerDefinitionProvider(latexSymbolSelector, lw_1.lw.language.definition), vscode.languages.registerDocumentSymbolProvider(latexSymbolSelector, lw_1.lw.language.docSymbol), vscode.languages.registerDocumentSymbolProvider(bibtexSelector, lw_1.lw.language.docSymbol), vscode.languages.registerDocumentSymbolProvider(selectDocumentsWithId(['doctex']), lw_1.lw.language.docSymbol), vscode.languages.registerWorkspaceSymbolProvider(lw_1.lw.language.projectSymbol));
    ...
    const registerTrigger = () => {
        const triggers = configuration.get('intellisense.triggers.latex');
        logger.log(`Trigger characters for intellisense of LaTeX documents: ${JSON.stringify(triggers)}`);
        triggerDisposable = vscode.languages.registerCompletionItemProvider(latexDoctexSelector, lw_1.lw.completion.provider, ...triggers);
    ...
    extensionContext.subscriptions.push(vscode.languages.registerCodeActionsProvider(latexSymbolSelector, lw_1.lw.lint.latex.actionprovider), vscode.languages.registerFoldingRangeProvider(latexSymbolSelector, lw_1.lw.language.folding), vscode.languages.registerFoldingRangeProvider(weaveSelector, lw_1.lw.language.weaveFolding), vscode.languages.registerFoldingRangeProvider(latexDoctexSelector, lw_1.lw.language.doctexFolding));

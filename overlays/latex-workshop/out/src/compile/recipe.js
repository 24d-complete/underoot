...
async function build(rootFile, langId, buildLoop, recipeName) {
    ...
    let cwd = path_1.default.dirname(lw_1.lw.file.toUri(rootFile).fsPath);
    const configuration = vscode_1.default.workspace.getConfiguration('latex-workshop');
    if (configuration.get('latex.build.fromWorkspaceFolder')) {
        const workspaceFolder = vscode_1.default.workspace.getWorkspaceFolder(lw_1.lw.file.toUri(rootFile));
        if (workspaceFolder) {
    ...
    // --- Underoot: Pre-build TeX check ---
    const texEnv = require('./tex-environment');
    const env = await texEnv.detectTexEnvironment();
    if (env.texDistro === 'none') {
        const tinyTexInstaller = require('./tinytex-installer');
        await tinyTexInstaller.promptInstallTinyTeX();
        // Re-check after potential install
        const envAfter = await texEnv.detectTexEnvironment();
        if (envAfter.texDistro === 'none') {
            logger.log('No TeX distribution found. Aborting build.');
            void vscode_1.default.window.showErrorMessage('No LaTeX engine found. Please install TinyTeX or configure a TeX distribution.');
            return;
        }
    }
    // --- End Underoot ---

    // --- Underoot: Static-Analysis Batch Downloader ---
    const PackageDownloader = require('./package-downloader');
    const packages = PackageDownloader.parseRequiredPackages(rootFile);
    await PackageDownloader.ensurePackagesInstalled(packages, rootFile);
    // --- End Underoot ---
    ...
    if (tools?.map(tool => tool.command).includes('latexmk') && rootFile === lw_1.lw.root.subfiles.path && lw_1.lw.root.file.path) {
        await createOutputSubFolders(lw_1.lw.root.file.path);
    }
    else {
    ...
    tools.forEach(tool => queue_1.queue.add(tool, rootFile, recipeName || 'Build', timestamp, false, cwd));
 ...
 * @param {string} rootFile - Path to the root LaTeX file.
 */
async function createOutputSubFolders(rootFile) {
    ...
    for (const file of lw_1.lw.cache.getIncludedTeX(rootFile)) {
...
async function createBuildTools(rootFile, langId, recipeName) {
    let buildTools = [];
    const configuration = vscode_1.default.workspace.getConfiguration('latex-workshop', lw_1.lw.file.toUri(rootFile));

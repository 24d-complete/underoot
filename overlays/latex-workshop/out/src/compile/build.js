...
exports.build = build;
exports.isFileExcludedFromBuildOnSave = isFileExcludedFromBuildOnSave;
 ...
 */
function isFileExcludedFromBuildOnSave(filePath) {
    const configuration = vscode.workspace.getConfiguration('latex-workshop', lw_1.lw.file.toUri(filePath));
    const globsToIgnore = configuration.get('latex.autoBuild.onSave.files.ignore');
...
async function build(skipSelection = false, rootFile = undefined, languageId = undefined, recipe = undefined) {
    ...
    if (rootFile === undefined && lw_1.lw.file.hasLaTeXLangId(activeEditor.document.languageId)) {
...
function spawnProcess(step) {
    ...
    if (!step.isExternal &&
        ...
        if (args && !step.name.endsWith(lw_1.lw.constant.MAGIC_PROGRAM_ARGS_SUFFIX)) {
            ...
            lw_1.lw.compile.process = lw_1.lw.external.spawn(`${step.command} ${args[0]}`, [], { cwd, env, shell: true });
        }
        else {
            lw_1.lw.compile.process = lw_1.lw.external.spawn(step.command, args ?? [], { cwd, env });
        }
    }
    else if (!step.isExternal) {
        if (step.command === 'latexmk' && step.rootFile === lw_1.lw.root.subfiles.path && lw_1.lw.root.dir.path && cwd === path.dirname(step.rootFile)) {
...
function handleExitCodeError(step, env, stderr, code, signal) {
    if (!step.isExternal) {
        ...
        logger.log(`Does the executable exist? $PATH: ${env['PATH']}, $Path: ${env['Path']}, $SHELL: ${process.env.SHELL}`);
        logger.log(`${stderr}`);
        lw_1.lw.parser.parse.log(stderr);
    }
    const configuration = vscode.workspace.getConfiguration('latex-workshop', step.rootFile ? lw_1.lw.file.toUri(step.rootFile) : undefined);

    // --- Underoot: Handle transitive missing packages ---
    if (!step.isExternal && stderr) {
        const missingMatch = stderr.match(/! LaTeX Error: File \`([^\']+)\' not found\./) || stderr.match(/Latexmk: Missing input file \'([^\']+)\'/);
        if (missingMatch && missingMatch[1]) {
            const missingFile = missingMatch[1];
            logger.log(`[Underoot] Dynamically caught missing transitive dependency: ${missingFile}`);

            const packageDownloader = require('./package-downloader');
            const pkgName = missingFile.replace(/\.(sty|cls)$/, '');

            vscode.window.withProgress({
                location: vscode.ProgressLocation.Notification,
                title: `Underoot Auto-Downloader`,
                cancellable: false
            }, async (progress) => {
                progress.report({ message: `Fetching transitive dependency: ${missingFile}...` });
                const resolved = await packageDownloader.resolvePackageName(pkgName);
                if (resolved) {
                    progress.report({ message: `Installing ${resolved}...` });
                    await new Promise(res => {
                        const proc = lw_1.lw.external.spawn('tlmgr', ['install', resolved], { env: process.env });
                        proc.on('exit', () => {
                            logger.log(`[Underoot] Installed transitive dependency ${resolved}`);
                            res();
                        });
                        proc.on('error', (err) => {
                            logger.log(`[Underoot] tlmgr error: ${err.message}`);
                            res();
                        });
                    });
                }

                // Then proceed to normal retry logic
                if (signal !== 'SIGTERM' && !step.isRetry && configuration.get('latex.autoBuild.cleanAndRetry.enabled')) {
            ...
            lw_1.lw.compile.process = undefined;
            return; // We handled the flow asynchronously
        }
    }
    // --- End Underoot ---
...
function handleRetryError(step) {
    ...
    // --- Underoot: Invalidate package cache so missing packages are re-checked ---
    try { require('./package-downloader').invalidateCache(step.rootFile); } catch { /* ignore */ }
    // --- End Underoot ---
